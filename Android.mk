...
GCC_LD := $(abspath prebuilts/gcc/linux-x86/aarch64/aarch64-linux-gnu/bin/)

KERNEL_SRC                  := $(abspath $(TARGET_KERNEL_SOURCE))
KERNEL_DEFCONFIG := $(TARGET_KERNEL_CONFIG)
KERNEL_CONFIG := $(KERNEL_OUT)/.config
KERNEL_TARGET_BINARY := $(KERNEL_OUT)/arch/$(TARGET_ARCH)/boot/Image
KERNEL_CFLAGS := HOSTCC=$(ANDROID_CLANG_TOOLCHAIN) ARCH=arm64
KERNEL_CFLAGS += CC=$(ANDROID_CLANG_TOOLCHAIN) HOSTLDFLAGS=-fuse-ld=lld LD=ld.lld LLVM=1 LLVM_IAS=1 AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump
KERNEL_CFLAGS += OBJSIZE=llvm-size READELF=llvm-readelf STRIP=llvm-strip CROSS_COMPILE=aarch64-linux-gnu- 
...
$(KERNEL_CONFIG): $(KERNEL_OUT)
	PATH=$(GCC_LD):$$PATH TEMPORARY_DISABLE_PATH_RESTRICTIONS=true  $(ANDROID_MAKE) -C $(KERNEL_SRC) O=$(KERNEL_OUT_ABS) $(KERNEL_CFLAGS) $(KERNEL_DEFCONFIG)

$(KERNEL_TARGET_BINARY): $(KERNEL_OUT) $(KERNEL_CONFIG) $(KERNEL_MODULES_OUT)
	PATH=$(GCC_LD):$$PATH $(ANDROID_MAKE) CONFIG_DEBUG_SECTION_MISMATCH=y -C $(KERNEL_SRC) O=$(KERNEL_OUT_ABS) $(KERNEL_CFLAGS) Image -j `$(NPROC)`
